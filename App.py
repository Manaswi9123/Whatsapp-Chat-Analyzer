...
if uploaded_file is not None:
    ...
    if st.sidebar.button('Show Analysis'):
        ...
        #wordcloud
        st.title('WordCloud')
        dfwc=helper.create_wordcloud(selected_user,df)
        fig, ax = plt.subplots()
        plt.imshow(dfwc)
        st.pyplot(fig)

        #most common words
        most_commondf=helper.most_common_words(selected_user,df)
        st.title('Most Common Words')
        fig,ax=plt.subplots()
        ax.barh(most_commondf[0], most_commondf[1],color='pink')
        plt.xticks(rotation='vertical')
        st.pyplot(fig)

        #emoji analysis
        emodf=helper.emoji_helper(selected_user,df)
        ...
        with col2:
            fig,ax=plt.subplots()
            ax.pie(emodf[1].head(),labels=emodf[0].head(),autopct='%0.2f')
            st.pyplot(fig)